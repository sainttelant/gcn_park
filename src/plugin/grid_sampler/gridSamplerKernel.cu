...
void grid_sample(T *output, const T *input, const T *grid, int64_t *output_dims,
  ...
  if (nb_dims == 4) {
    ...
    cudaError_t err_sampler_2d = cudaGetLastError();
    if (cudaSuccess != err_sampler_2d) {
      fprintf(stderr, "cudaCheckError() grid_sampler_2d_kernel failed: %s\n",
              cudaGetErrorString(err_sampler_2d));
      exit(-1);
    }
  } else if (nb_dims == 5) {
